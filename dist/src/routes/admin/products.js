...
import { validateProductPatchBody, validateProductPostBody, } from "../../middlewares/product_validation.js";
...
productRouter.post("/products", multerUpload.single("file"), validateProductPostBody, async (request, response) => {
    const { body } = request;
...
productRouter.patch("/products/:id", validateProductPatchBody, async (request, response) => {
    const { body } = request;
    const parsedId = parseInt(request.params.id);
    const result = validationResult(request);
    if (isNaN(parsedId))
